...
CON
        ...
        _CLK1MS = _CLKFREQ / 1000                       '100,000

        _CLKTICKS = 100                                 '100
        _CLKCLKS = _CLK1MS / _CLKTICKS                  '1000
...
Pub Start
    ...
    lockset(InitLock)                                   'InitLock prevents cogs from running 'til TimerCog
                                                        '. has built the global area                        
...
'
'       the following code is a copy of the code in urcvlong that receives the first byte of the next word
'
...
:byt1                   test    USBRxFmask,ina          wz                           
              ...
              if_z      jmp     utxmrtn
                        ...
                        jmpret  urcvlong_ret,#urcvcmd    'Wait
...
:byt4                   test    USBRxFmask,ina          wz
              if_nz     jmp     #urcvtmo
                        ...
                        add     urcvdata,#1             wz, nr                  'rcvd 0xffffffff?
              if_z      jmp     #urcvfoxes                                      '. yes - check for twice
                        
                        mov     urcvffcnt,#2

urcvcont                test    $,#0                    wc                      'Clear carry                        
...
urcvfoxes
                        djnz    urcvffcnt,#urcvcont                             '. yes - but not twice yet
...
'
:spin1                  lockset ufreelock       wc      'Grab the lock - busy
              if_c      jmp     #:spin1                 '. Yes - spin

                        rdword  ut1,#GblFreeChain       'Load @'next buffer

                        wrword  ut1,urcvbufr               'Chain
                        
                        wrword  urcvbufr,#GblFreeChain     '. the buffer

                        rdbyte  urcvfreecnt,#GblFreeCnt 'Increment
                        add     urcvfreecnt,#1          '. the
                        wrbyte  urcvfreecnt,#GblFreeCnt '. . free buffer count
...
'
'       Check for all cogs in expected state
'
chkstate
                        mov     usc0,ut0
...
:init                   lockset binitlock           wc
              if_c      jmp     #:init
                        ...
                        add     chan2head,chan2chan     'Compute @'busy buffer
                        add     chan2tail,chan2chan     'Compute @'free buffer
                        ...
                        add     chan3head,chan3chan     'Compute @'busy buffer
                        add     chan3tail,chan3chan     'Compute @'free buffer
                        ...
                        add     chan4head,chan4chan     'Compute @'busy buffer
                        add     chan4tail,chan4chan     'Compute @'free buffer
...
chan1top
              ...
              if_z      jmp     #chan1ret               '. Yes, == PAUSE ==
                        ...
                        add     chan1bfr,#BfrTimer      'Load
                        rdlong  bt1,chan1bfr       wz   '. time from buffer - zero?
                        sub     chan1bfr,#BfrTimer      ' (restore @'buffer)
              if_z      jmp     #:gotone                '. Yes - process the data

                        rdlong  bt2,#GblTimer           'Load current time
                        cmp     bt1,bt2         wz,wc   'Is it time for this data?
              if_a      jmp     #chan1ret               '. No, don't busy it yet
...
:idle                                               
                        ...
                        djnz    chan1bits,#chan1ret     'Loop for all 'quiet'
              ...
              if_c      jmp     #chan1ret
                        ...
                        rdword  bt1,chan1bfr    wz
                        wrword  bt1,chan1head
              if_z      wrword  bt1,chan1tail
...
:stp1                        
                        lockset bfreelock       wc
              if_c      jmp     #chan1ret

                        rdword  bt2,#GblFreeChain
                        wrword  bt2,chan1bfr
                        wrword  chan1bfr,#GblFreeChain
...
:stp2
              ...
              if_c      jmp     #chan1ret
                        ...
                        rdbyte  bt2,#GblFreeCnt         'Increment
                        add     bt2,#1                  '. the
                        wrbyte  bt2,#GblFreeCnt         '. . free buffer count
...
chan2top
              ...
              if_nz     jmp     #chan2ret               '. No - just wait
                        ...
                        rdword  chan1bfr,chan1head  wz  'Load head pointer - zero?   
              if_z      jmp     #chan1ret               '. Yes, == PAUSE ==
...
:idle                                               
              ...
              if_c      jmp     #chan2ret
                        ...
                        rdword  bt1,chan2bfr    wz
                        wrword  bt1,chan2head
              if_z      wrword  bt1,chan2tail
...
chan3top
              ...
              if_nz     jmp     #chan3ret               '. No - just wait
                        ...
                        rdword  chan3bfr,chan3head  wz  'Load head pointer - zero?   
              if_z      jmp     #chan3ret               '. Yes, == PAUSE ==

                        add     chan3bfr,#BfrTimer      'Load
                        rdlong  bt1,chan3bfr       wz   '. time from buffer - zero?
                        sub     chan3bfr,#BfrTimer      ' (restore @'buffer)
              if_z      jmp     #:gotone                '. Yes - process the data

                        rdlong  bt2,#GblTimer           'Load current time
                        cmp     bt1,bt2         wz,wc   'Is it time for this data?
              if_a      jmp     #chan3ret               '. No, don't busy it yet
...
:idle                                               
              ...
              if_c      jmp     #chan3ret
                        ...
                        rdword  bt1,chan3bfr    wz
                        wrword  bt1,chan3head
              if_z      wrword  bt1,chan3tail
...
chan4top
              ...
              if_nz     jmp     #chan4ret               '. No - just wait
                        ...
                        rdword  chan4bfr,chan4head  wz  'Load head pointer - zero?   
              if_z      jmp     #chan4ret               '. Yes, == PAUSE ==

                        add     chan4bfr,#BfrTimer      'Load
                        rdlong  bt1,chan4bfr       wz   '. time from buffer - zero?
                        sub     chan4bfr,#BfrTimer      ' (restore @'buffer)
              if_z      jmp     #:gotone                '. Yes - process the data

                        rdlong  bt2,#GblTimer           'Load current time
                        cmp     bt1,bt2         wz,wc   'Is it time for this data?
              if_a      jmp     #chan4ret               '. No, don't busy it yet
...
:idle                                               
              ...
              if_c      jmp     #chan4ret
                        ...
                        rdword  bt1,chan4bfr    wz
                        wrword  bt1,chan4head
              if_z      wrword  bt1,chan4tail
...
chan2head               long    ChanTblHead
chan2tail               long    ChanTblTail
...
chan3head               long    ChanTblHead
chan3tail               long    ChanTblTail
...
chan4head               long    ChanTblHead
chan4tail               long    ChanTblTail
