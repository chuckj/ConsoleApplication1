...
namespace SharpHelper
{
    ...
    {
        private static log4net.ILog logger = log4net.LogManager.GetLogger(nameof(SharpDevice));
        ...
        {
            construct(form, Format.R8G8B8A8_UNorm, debug);
        }
        ...
        /// </summary>
        /// <param name="form">Rendering form</param>
        /// <param name="format">Rendering form</param>
        /// <param name="debug">Active the debug mode</param>
        public SharpDevice(RenderControl form, Format format, bool debug = false)
        {
            construct(form, format, debug);
        }
        ...
        { 
            ...
            var desc = new SwapChainDescription()
            {
                ...
                ModeDescription = new ModeDescription() { Width = sz.Width, Height = sz.Height, /*new Rational(60, 1),*/ Format = format },//sview
                IsWindowed = true,
        ...
        public static IntPtr GetHandle(RenderControl C)
        {
            if (!C.InvokeRequired) return C.Handle;
            else return (IntPtr)C.Invoke(new Func<RenderControl, IntPtr>(GetHandle), C);
        }

        public static SD.Size GetSize(RenderControl C)
        {
            if (!C.InvokeRequired) return C.ClientSize;
            else return (SD.Size)C.Invoke(new Func<RenderControl, SD.Size>(GetSize), C);
        }
        ...
        {
            Console.WriteLine(SharpDX.Direct3D11.Device.GetSupportedFeatureLevel().ToString());
            //Console.ReadLine();
            return SharpDX.Direct3D11.Device.GetSupportedFeatureLevel() /*== FeatureLevel.Level_11_0*/  >= FeatureLevel.Level_10_0;
        }
